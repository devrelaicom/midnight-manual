...
mod tests {
    ...
    /// `mn-store` migration SQL. Returns the values listed by the LAST
    ...
    fn migration_event_types() -> Option<Vec<String>> {
        // crates/mn-telemetry -> crates/mn-store/migrations
    ...
    fn event_type_set_matches_migration_check_constraint() {
        ...
        let Some(mut from_migration) = migration_event_types() else {
            ...
            eprintln!("skipping: mn-store migrations not found from mn-telemetry");
            return;
        };
        ...
        assert_eq!(
            ...
            "EventPayload event_type set diverged from the migration CHECK constraint \
             (mn-store/migrations); a new event_type needs a migration extending the constraint \
