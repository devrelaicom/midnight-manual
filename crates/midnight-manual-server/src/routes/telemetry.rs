...
//! (FR-110 / FR-111).
//!
//! Accepts a JSON array of typed events (see `mnm_telemetry::Event`). Validates
...
/// `mnm_telemetry::EventPayload::event_type()`. The server-side validator MUST
...
/// One row in the inbound batch. Fields mirror `mnm_telemetry::Event` with the
...
mod tests {
    use super::*;
    use mnm_telemetry::events::{Component, Event, EventPayload};
    ...
    fn allow_list_covers_every_event_payload_variant() {
        ...
        let variants = [
            EventPayload::McpToolCall {
                tool_name: mnm_telemetry::events::McpToolName::Search,
                latency_ms: 0,
                result_count: 0,
                model_state: mnm_telemetry::events::ModelState::Ready,
                rerank_on: false,
                outcome: mnm_telemetry::events::Outcome::Ok,
                corpus_model: None,
                reranker_used: None,
                top_confidence: None,
            ...
            EventPayload::CliCommand {
                command: mnm_telemetry::events::CliCommandName::Search,
                duration_ms: 0,
                outcome: mnm_telemetry::events::Outcome::Ok,
            },
            ...
            EventPayload::McpStartup {
                startup_ms: 0,
                model_state: mnm_telemetry::events::ModelState::Missing,
            },
            EventPayload::McpShutdown { uptime_s: 0, tools_served: 0 },
        ];
