...
impl TokenUsageLimiter {
    ...
    pub async fn refresh_overrides_now(
        &self,
        pool: &sqlx::PgPool,
    ) -> Result<usize, mnm_store::error::StoreError> {
        let rows = mnm_store::entities::token_limit_override::list_active(pool).await?;
...
mod tests {
    ...
    fn test_admin_ctx(sub: &str) -> crate::middleware::bearer::AuthContext {
        crate::middleware::bearer::AuthContext {
            sub: sub.to_owned(),
            role: mnm_auth::Role::Admin,
            tier: mnm_auth::Tier::Admin,
            jti: String::new(),
        }
    }
