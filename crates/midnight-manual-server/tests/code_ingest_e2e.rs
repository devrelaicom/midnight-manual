...
//! `crates/mnm-content/tests/corpus/`. It verifies four observable properties
...
async fn code_ingest_smoke_persists_symbol_paths_and_packages() {
    ...
    //
    // CARGO_MANIFEST_DIR resolves to `crates/midnight-manual-server` at compile time; the
    // corpus lives at `../mnm-content/tests/corpus` relative to that.
