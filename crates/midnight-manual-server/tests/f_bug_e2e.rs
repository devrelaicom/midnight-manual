...
//!
//! The F-bug (§3.4 of the ingest UX design) was that `midnight_manual`'s `DocumentUpload`
...
//! `mnm_content::language` lookup + tokens approximation) through
//! `PlannedDocument` and onto the wire.
//!
//! The existing in-tree regression (`crates/midnight-manual/tests/ingest_integration.rs::
...
//!
//! This test fills that gap: it boots a real `midnight-manual-server` axum app against a
//! real Postgres+pgvector (via `common::boot()`), drives the actual
//! `midnight_manual::commands::ingest::run::run_with_paths` CLI entry point against
...
use midnight_manual::commands::ingest::run::{run_with_paths, Args as IngestArgs};
use midnight_manual_server::{app, config::ServerConfig};
use mnm_auth::{mint_jwt, Claims, Keypair, Role, SigningSecret, DEFAULT_ADMIN_TTL};
...
async fn spawn_server(pool: sqlx::PgPool, cfg: ServerConfig, voyage_mock_uri: &str) -> String {
    ...
    let voyage_ctx = Some(Arc::new(
        mnm_embedding::contextualized::ContextualizedVoyageEmbedder::new(
            "test-key",
            "voyage-context-3",
            1024,
            "float",
    ...
    let code_model = Arc::new(RwLock::new(
        midnight_manual_server::code_model::resolve(&pool, "voyage-code-3@1")
            .await
            .ok(),
    ));
...
/// - `language`      = `markdown` (from `mnm_content::language::from_path`)
...
async fn document_metadata_persists_to_postgres_through_full_ingest() {
    ...
    std::fs::write(
        ...
        // `mnm_content::manifest::resolve` inheritance. The resolved leaf
    ...
    let args = IngestArgs {
        ...
        embedding_model: midnight_manual::commands::ingest::run::DEFAULT_EMBEDDING_MODEL.to_owned(),
    ...
    for row in &rows {
        ...
        // language — F-bug field #3. `mnm_content::language::from_path`
