...
//!
//! Exercises the job-layer wrapper `midnight_manual_server::jobs::source_retention` so
//! its `sweep_once` correctly converts the `grace_hours` config into the
//! `grace_seconds` the store-level helper takes, and so the
//! [`midnight_manual_server::jobs::source_retention::SweepStats`] shape is preserved.
...
use midnight_manual_server::jobs::source_retention;
...
async fn seed_retired_source(pool: &PgPool, prefix: &str, retired_seconds_ago: i64) -> String {
    ...
    let doc_id = document::insert(
        ...
        document::NewDocument {
            ...
            frontmatter: None,
            provenance: &mnm_core::provenance::Provenance::default(),
            package_id: None,
            char_count: 1,
            token_count: 1,
            license: None,
...
async fn sweep_once_runs_version_pass_alongside_source_pass() {
    ...
    use mnm_store::entities::{embedding_model, source as source_entity, source_version};
    ...
    let remaining = mnm_store::entities::source_version::list_for_source(&h.pool, source_id)
        .await
        .unwrap();
...
async fn sweep_once_runs_aborted_pass_alongside_other_passes() {
    ...
    assert!(
        matches!(aborted_row, Err(mnm_store::StoreError::NotFound)),
