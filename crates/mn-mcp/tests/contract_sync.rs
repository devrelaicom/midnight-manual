...
fn mcp_tools_json_mirrors_tools_list() {
    ...
    let error_schema = mn_mcp::schemas::error_output_schema();
...
fn error_codes_agree_across_code_schema_and_prose() {
    ...
    let from_code: BTreeSet<String> = mn_mcp::render::ErrorKind::ALL
        .iter()
        .map(|k| k.code().to_owned())
        .collect();

    let schema = mn_mcp::schemas::error_output_schema();
