...
The `midnight-manual` crate provides the `mnm` command-line interface. Use it to ingest
documents, query the search index, and manage sources.

### The MCP Server

The `mnm-mcp` crate exposes a Model Context Protocol server over stdio. It is
launched by AI clients (like Claude Code) to answer queries against the indexed
corpus.

### The Cloud Server

The `midnight-manual-server` crate is a Fly.io-deployed HTTP API that backs both the CLI
