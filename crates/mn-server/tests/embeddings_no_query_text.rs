...
//! handler ([`mn_server::routes::embeddings`]) was built to never log or persist
...
//! `mn_server` target) — a canary that cannot fail. Part C drives a request at a
...
//! genuinely sees mn-server's `tracing` output, so a future success-path
...
//! [`mn_telemetry::canary::CANARY_STRINGS`]
...
//! text. We must assert on what *mn-server* emits, not on what the mock
...
//! fails iff mn-server logs the input, which is exactly the invariant.
...
async fn embeddings_path_emits_no_query_text() {
    ...
    // *mn-server* logs the input — exactly the invariant under test.
    let captured = CapturedLogs::default();
    let filter = EnvFilter::new(
        "off,mn_server=trace,mn_embedding=trace,mn_telemetry=trace,mn_retrieval=trace,mn_store=trace,mn_core=trace,mn_auth=trace",
    );
    ...
    // typo'd `mn_server` target), the buffer would be silently empty and a real
    ...
    // mn-server's `tracing` output (see its doc comment). It runs on a FRESH,
...
/// Positive control: prove the captured-logs pipeline actually sees mn-server's
...
/// pass against an empty buffer if the `EnvFilter`/`mn_server` target scoping
...
/// `mn_server::routes::embeddings` target) and mapping to a 502, then asserts:
///
...
async fn positive_control_voyage_failure(pool: sqlx::PgPool, sentinel: &str) {
    ...
    // POSITIVE CONTROL: the capture pipeline DID see mn-server's `tracing`
    // output. If this fails, the `EnvFilter`/`mn_server` target scoping is wrong
