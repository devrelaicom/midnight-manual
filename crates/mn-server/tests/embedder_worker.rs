...
use mn_server::jobs::embedder::{embed_once_scoped, EmbedFn, EmbedFuture};
...
async fn seed_embed_failed_chunks(pool: &PgPool, count: usize) -> (Uuid, Uuid, Vec<Uuid>) {
    // Use the canonical seeded model so the read_endpoints test continues
    // to see bge-base as `get_active` (creating a per-test model row
    // perturbs the ORDER BY in `get_active`). Tests scope by the returned
    // sv_id instead via `embed_once_scoped`.
...
async fn embed_once_promotes_embed_failed_to_ready() {
    ...
    let promoted = embed_once_scoped(&h.pool, &emb, model_id, Some(sv_id), 16)
        .await
        .unwrap();
...
async fn embed_once_respects_batch_size() {
    ...
    let promoted = embed_once_scoped(&h.pool, &emb, model_id, Some(sv_id), 2)
        .await
        .unwrap();
...
async fn embed_once_skips_chunks_with_different_model() {
    let h = common::boot().await;
    let (sv_id, _seeded_model, _) = seed_embed_failed_chunks(&h.pool, 3).await;
    // Ask the worker to process an unrelated model id — there are no
    // embed_failed chunks under it so promoted = 0 even though our sv_id
    // owns 3 of them under the seeded model.
    ...
    let promoted = embed_once_scoped(&h.pool, &emb, other_id, Some(sv_id), 16)
        .await
        .unwrap();
...
async fn empty_batch_returns_zero_without_calling_embedder() {
    ...
    let model_id = embedding_model::upsert(&h.pool, "bge-base-en-v1.5", 1, 768, "baai")
        .await
        .unwrap();
    // Use a sv_id we never seeded so the filter returns nothing.
    let phantom_sv = Uuid::new_v4();
    let calls = Arc::new(AtomicUsize::new(0));
    let emb = ConstantEmbedder {
        dim: 768,
        fill: 0.0,
    ...
    let promoted = embed_once_scoped(&h.pool, &emb, model_id, Some(phantom_sv), 16)
        .await
        .unwrap();
    assert_eq!(promoted, 0);
...
async fn promoted_chunks_are_visible_to_search_path() {
    ...
    // Note: we intentionally do NOT finalize this source_version. Making it
    // active here would race with `read_endpoints::active_model_returns_seeded_row`
    // which expects the seeded bge-base model to be `get_active`'s pick.
    // `get_by_id_ready` doesn't check is_active, only chunk.status — so the
    // post-embed visibility check works without promotion.
