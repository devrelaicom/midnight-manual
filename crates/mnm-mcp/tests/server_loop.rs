...
use mnm_mcp::protocol;
use mnm_mcp::transport::{frame_blocking, FrameReader, FrameWriter};
...
/// invoke `mnm_mcp::run` directly (it expects stdin/stdout); instead we
...
async fn prompts_list_and_get_through_framed_io() {
    ...
    let list_result = mnm_mcp::prompts::list();
    assert!(
        list_result
            .prompts
            .iter()
