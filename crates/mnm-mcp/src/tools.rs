    ...
    ToolsListResult {
        tools: vec![
            ToolDescription {
                name: "search",
                description:
                    "Search the Midnight Network documentation and code corpus (docs, SDK references, Compact language material, code examples). Returns ranked excerpts with confidence scores and source attribution. Use it whenever you need facts about Midnight, Compact, or the Midnight SDK. Code-heavy queries (function names, API signatures, error strings from code) benefit from code_mode=exclusive; conceptual queries should keep the default. For multi-query strategies, facet filters, or rerank control, use advanced_search. In results, symbol_path is a flat breadcrumb of symbol names; the get_chunks family returns the richer structured {kind, name} segments.",
            ...
            ToolDescription {
                ...
                description:
                    "Full-control search over the Midnight corpus: fuse multiple queries (HyDE, expansion, step-back), restrict by facet filters, switch retrieval mode, and toggle reranking. Use when basic search comes up short or when the midnight-advanced-search skill prescribes a pattern. Call facets first to discover valid filter values. In results, symbol_path is a flat breadcrumb of symbol names; the get_chunks family returns the richer structured {kind, name} segments.",
            ...
            ToolDescription {
                name: "get_chunks",
                description:
                    "Fetch the full content of one or more chunks by id, typically ids returned by search. Use this to read the actual text behind search results. For code chunks, symbol_path is an array of structured {kind, name} segments (search returns only the flat name breadcrumb).",
            ...
            ToolDescription {
                name: "get_chunk_next",
                description:
                    "Fetch chunks that immediately follow a given chunk in its document's reading order. Use to continue reading past the end of a chunk you already have. Code chunks carry the structured {kind, name} symbol_path segments.",
            ...
            ToolDescription {
                name: "get_chunk_prev",
                description:
                    "Fetch chunks that immediately precede a given chunk in its document's reading order. Use to read the context leading up to a chunk you already have. Code chunks carry the structured {kind, name} symbol_path segments.",
                input_schema: chunk_nav_schema(),
                output_schema: Some(crate::schemas::chunk_list_output_schema()),
            ...
            ToolDescription {
                ...
                description:
                    "Fetch the chunks immediately before and after a given chunk in one call. Use when a search hit needs surrounding context to be understood. Code chunks carry the structured {kind, name} symbol_path segments.",
            ...
            ToolDescription {
                ...
                description:
                    "Read a window of a document's chunk bodies by position. Use after get_document to read a document section by section. Code chunk bodies include the structured {kind, name} symbol_path segments (same shape as the get_chunks family).",
