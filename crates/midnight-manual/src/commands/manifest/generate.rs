...
use mnm_content::manifest::generate::{generate, GenerateOptions};
use mnm_content::manifest::{robots, sitemap};
...
pub async fn run(args: Args) -> Result<()> {
    ...
    let body =
        mnm_content::manifest::generate::emit_yaml(&result.manifest, &date).context("emit yaml")?;
        ...
        if sitemap::is_http(spec) {
            ...
            let urls_to_load = if robots::looks_like_sitemap_spec(spec) {
                ...
                if discovered.is_empty() {
                    tracing::debug!(
                        target: "midnight-manual::manifest::generate",
                        spec = %spec,
