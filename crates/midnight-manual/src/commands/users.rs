...
//! `crates/mnm-core/src/paths.rs` and `crates/midnight-manual-server/src/config.rs`.)
...
use mnm_auth::{parse_public_key_wire, Role};
...
/// Role choices on the CLI side. Mirrors [`mnm_auth::Role`] but stays a
...
fn resolve_user_store_path() -> Result<PathBuf> {
    mnm_core::paths::user_store_path(&mnm_core::config::StdEnv).ok_or_else(|| {
        anyhow!(
...
/// On-disk TOML row. Mirrors [`mnm_auth::User`] but stays a CLI-local type so
...
impl StoreFile {
    fn read(path: &Path) -> Result<Self> {
        if !path.exists() {
            return Ok(Self {
                schema_version: mnm_auth::USER_STORE_SCHEMA_VERSION,
                users: Vec::new(),
            });
        }
        ...
        if file.schema_version > mnm_auth::USER_STORE_SCHEMA_VERSION {
            return Err(anyhow!(
                "user store schema_version={} is newer than supported (max {})",
                file.schema_version,
                mnm_auth::USER_STORE_SCHEMA_VERSION,
            ));
        }
        Ok(file)
    }
...
mod tests {
    ...
    fn fixture_public_key() -> String {
        mnm_auth::Keypair::generate().public_wire()
    }
    ...
    fn write_then_read_round_trips_via_mn_auth_loader() {
        ...
        // shape and `mnm_auth::UserStore::parse`.
        let dir = tempfile::tempdir().unwrap();
        let path = fresh_store(dir.path());
        let pk = fixture_public_key();
        add(
        ...
        .unwrap();
        let body = std::fs::read_to_string(&path).unwrap();
        let store = mnm_auth::UserStore::parse(&body).expect("strict parse succeeds");
