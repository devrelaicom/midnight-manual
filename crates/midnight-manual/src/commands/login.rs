...
//!    `[admin]` via [`mnm_core::auth_file::AuthFile::write_admin_token`], which
...
pub async fn run(
    ...
    let private_path = mnm_core::paths::private_key_path(&env, &args.user_id).ok_or_else(|| {
        anyhow!(
...
pub async fn run_with_paths(
    ...
    if !dry_run {
        mnm_core::auth_file::AuthFile::write_admin_token(
            auth_path,
            &verified.user_id,
            &verified.token,
            expires_at,
