...
//! exchange is covered separately in `crates/mn-server/tests/github_oauth.rs`.
...
"#,
    ...
    mn_cli::commands::auth::status_with_path(&path, true).unwrap();
}
...
fn status_handles_absent_auth_file() {
    ...
    mn_cli::commands::auth::status_with_path(&path, false).unwrap();
}
...
"#,
    ...
    mn_cli::commands::auth::logout_with_path(&path, true).unwrap();
...
async fn local_listener_captures_oauth_callback() {
    ...
    let listener_task = tokio::spawn(async move {
        mn_cli::commands::auth::run_with_paths(&listener, Duration::from_secs(10))
            .await
            .unwrap()
    });
...
async fn local_listener_ignores_non_oauth_paths() {
    ...
    let port = listener.local_addr().unwrap().port();
    let listener_task = tokio::spawn(async move {
        mn_cli::commands::auth::run_with_paths(&listener, Duration::from_secs(5))
            .await
            .unwrap()
    });
    tokio::time::sleep(Duration::from_millis(50)).await;
...
fn persist_read_uplift_writes_0600_on_unix() {
    ...
    mn_cli::commands::auth::persist_read_uplift(&path, "aaron", "ru-bearer-xyz", exp).unwrap();
