...
fn auth_path() -> Result<std::path::PathBuf> {
    mn_core::paths::auth_file_path(&mn_core::config::StdEnv)
...
pub fn status_with_path(path: &Path, json: bool) -> Result<()> {
    let file = mn_core::auth_file::AuthFile::read_optional(path)?
        .unwrap_or_else(mn_core::auth_file::AuthFile::empty);
...
pub fn logout_with_path(path: &Path, json: bool) -> Result<()> {
    let mut file = match mn_core::auth_file::AuthFile::read_optional(path)? {
        Some(f) => f,
        None => mn_core::auth_file::AuthFile::empty(),
    };
...
async fn github(args: GithubArgs, server_flag: Option<&str>, json: bool) -> Result<()> {
    ...
    if !args.dry_run {
        mn_core::auth_file::AuthFile::write_read_uplift_token(
            &auth_path,
            &github_login,
            &token,
            expires_at,
...
/// # Errors
///
/// Same as [`mn_core::auth_file::AuthFile::write_read_uplift_token`].
