...
use mnm_telemetry::client::Client;
use mnm_telemetry::events::{Component, EventPayload, ModelState};
use mnm_telemetry::{Event, NoopClient};
use tracing::Level;
...
fn mcp_tool_call_search_fields_do_not_leak_canary() {
    use mnm_telemetry::events::{McpToolName, Outcome};
...
fn rerank_event_fields_do_not_leak_canary() {
    ...
    // closed `mnm_core::rerank::known_reason` allow-list before it reaches the
