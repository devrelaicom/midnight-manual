...
Your AI assistant's training data went stale the day it shipped. Ask it a question about Midnight and it answers from whatever snapshot of the docs existed at training time, which can be months or years behind the current SDK, compiler, and contract patterns. midnight-manual fixes that.

## Ask your docs, not your model

midnight-manual is a retrieval engine built for the Midnight Network. It gives your AI assistant live search over the real Midnight docs and source code, so every answer traces back to a source you can open and check. Every result points to the exact doc or source file it came from.
...
- **A hosted corpus.** The indexed corpus is hosted by default, so most users never run a server. Search works the moment you install.

## Grounded, ranked, and current

Every result is retrieved from the live corpus and scored on three axes:

| Property | What it means |
|---|---|
| **Cited** | Every result points to the exact doc or source file, so your assistant shows its work. |
| **Ranked by trust** | Results are weighted by source, verification status, and freshness, so the most trustworthy source rises to the top. |
