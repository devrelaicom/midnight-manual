...
One long-running server can back several clients at once — useful when stdio would spawn a process per client, or when the server runs in a container. To poke at it interactively, point the [MCP Inspector](https://github.com/modelcontextprotocol/inspector) (`npx @modelcontextprotocol/inspector`) at `http://127.0.0.1:2400/mcp` with the transport set to Streamable HTTP; the server answers browser preflights and grants loopback origins, so both the Inspector UI and its CLI mode work.
