...
1. **Environment variable:** set `MIDNIGHT_MANUAL_DISABLE_TELEMETRY=1`.
2. **Config flag:** set `telemetry.enabled = false` in your config file.
3. **Runtime command:** run `mnm telemetry disable` — this writes a persistent marker that is read at every startup. Reverse it with `mnm telemetry enable`. Inspect the current state with `mnm telemetry status`.
